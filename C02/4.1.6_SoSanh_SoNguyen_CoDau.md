# 4.1.6 SO SÁNH SỐ NGUYÊN CÓ DẤU

## Mô tả dạng bài tập
So sánh hai số nguyên có dấu và xác định kết quả (lớn hơn, nhỏ hơn, bằng nhau).

## Phương pháp giải

### Phương pháp 1: So sánh trực tiếp
1. **So sánh hai số** như bình thường
2. **Xác định kết quả** (>, <, =)
3. **Kiểm tra** phạm vi giá trị

### Phương pháp 2: So sánh bit
1. **So sánh từng bit** từ trái sang phải
2. **Bit đầu tiên khác nhau** quyết định kết quả
3. **Tất cả bit giống nhau** → bằng nhau

## Bảng so sánh:
| A | B | A > B | A < B | A = B |
|---|---|-------|-------|-------|
| 5 | 3 | ✓     | ✗     | ✗     |
| 3 | 5 | ✗     | ✓     | ✗     |
| 5 | 5 | ✗     | ✗     | ✓     |
| -5 | -3 | ✗     | ✓     | ✗     |
| -3 | -5 | ✓     | ✗     | ✗     |
| -5 | -5 | ✗     | ✗     | ✓     |

---

## BÀI TẬP CÓ LỜI GIẢI CHI TIẾT (10 bài)

### Bài 1: So sánh 5 và 3 trong 4 bit có dấu

**Lời giải:**
```
5₁₀ = 0101₂
3₁₀ = 0011₂

So sánh từng bit:
Vị trí 3: 0 = 0
Vị trí 2: 1 > 0 → 5 > 3

Kết quả: 5 > 3 (4 bit có dấu)
```

### Bài 2: So sánh 3 và 5 trong 4 bit có dấu

**Lời giải:**
```
3₁₀ = 0011₂
5₁₀ = 0101₂

So sánh từng bit:
Vị trí 3: 0 = 0
Vị trí 2: 0 < 1 → 3 < 5

Kết quả: 3 < 5 (4 bit có dấu)
```

### Bài 3: So sánh 5 và 5 trong 4 bit có dấu

**Lời giải:**
```
5₁₀ = 0101₂
5₁₀ = 0101₂

So sánh từng bit:
Vị trí 3: 0 = 0
Vị trí 2: 1 = 1
Vị trí 1: 0 = 0
Vị trí 0: 1 = 1

Tất cả bit giống nhau → 5 = 5

Kết quả: 5 = 5 (4 bit có dấu)
```

### Bài 4: So sánh -5 và -3 trong 4 bit có dấu

**Lời giải:**
```
-5₁₀ = 1101₂ (bit dấu 1, phần trị 101₂ = 5)
-3₁₀ = 1011₂ (bit dấu 1, phần trị 011₂ = 3)

So sánh từng bit:
Vị trí 3: 1 = 1 (cùng âm)
Vị trí 2: 1 > 0 → phần trị của -5 (101₂) > phần trị của -3 (011₂)
→ Trong Signed Magnitude, số âm có phần trị lớn hơn thì nhỏ hơn
→ -5 < -3

Kết quả: -5 < -3 (4 bit có dấu, Signed Magnitude)
```

### Bài 5: So sánh -3 và -5 trong 4 bit có dấu

**Lời giải:**
```
-3₁₀ = 1011₂ (bit dấu 1, phần trị 011₂ = 3)
-5₁₀ = 1101₂ (bit dấu 1, phần trị 101₂ = 5)

So sánh từng bit:
Vị trí 3: 1 = 1 (cùng âm)
Vị trí 2: 0 < 1 → phần trị của -3 (011₂) < phần trị của -5 (101₂)
→ Trong Signed Magnitude, số âm có phần trị nhỏ hơn thì lớn hơn
→ -3 > -5

Kết quả: -3 > -5 (4 bit có dấu, Signed Magnitude)
```

### Bài 6: So sánh 50 và 100 trong 8 bit có dấu

**Lời giải:**
```
50₁₀ = 00110010₂ (bit dấu 0, phần trị 0110010₂ = 50)
100₁₀ = 01100100₂ (bit dấu 0, phần trị 1100100₂ = 100)

So sánh từng bit:
Vị trí 7: 0 = 0 (cùng dương)
Vị trí 6: 0 < 1 → 50 < 100

Kết quả: 50 < 100 (8 bit có dấu, Signed Magnitude)
```

### Bài 7: So sánh 100 và 50 trong 8 bit có dấu

**Lời giải:**
```
100₁₀ = 01100100₂ (bit dấu 0, phần trị 1100100₂ = 100)
50₁₀ = 00110010₂ (bit dấu 0, phần trị 0110010₂ = 50)

So sánh từng bit:
Vị trí 7: 0 = 0 (cùng dương)
Vị trí 6: 1 > 0 → 100 > 50

Kết quả: 100 > 50 (8 bit có dấu, Signed Magnitude)
```

### Bài 8: So sánh -50 và -100 trong 8 bit có dấu

**Lời giải:**
```
-50₁₀ = 10110010₂ (bit dấu 1, phần trị 0110010₂ = 50)
-100₁₀ = 11100100₂ (bit dấu 1, phần trị 1100100₂ = 100)

So sánh từng bit:
Vị trí 7: 1 = 1 (cùng âm)
Vị trí 6: 0 < 1 → phần trị của -50 (0110010₂) < phần trị của -100 (1100100₂)
→ Trong Signed Magnitude, số âm có phần trị nhỏ hơn thì lớn hơn
→ -50 > -100

Kết quả: -50 > -100 (8 bit có dấu, Signed Magnitude)
```

### Bài 9: So sánh 1000 và 2000 trong 16 bit có dấu

**Lời giải:**
```
1000₁₀ = 0000001111101000₂ (bit dấu 0, phần trị 000001111101000₂ = 1000)
2000₁₀ = 0000011111010000₂ (bit dấu 0, phần trị 000011111010000₂ = 2000)

So sánh từng bit:
Vị trí 15: 0 = 0 (cùng dương)
Vị trí 14: 0 = 0
Vị trí 13: 0 = 0
Vị trí 12: 0 = 0
Vị trí 11: 0 = 0
Vị trí 10: 0 = 0
Vị trí 9: 0 = 0
Vị trí 8: 0 = 0
Vị trí 7: 0 = 0
Vị trí 6: 0 = 0
Vị trí 5: 0 = 0
Vị trí 4: 0 = 0
Vị trí 3: 0 < 1 → 1000 < 2000

Kết quả: 1000 < 2000 (16 bit có dấu, Signed Magnitude)
```

### Bài 10: So sánh -1000 và -2000 trong 16 bit có dấu

**Lời giải:**
```
-1000₁₀ = 1000001111101000₂ (bit dấu 1, phần trị 000001111101000₂ = 1000)
-2000₁₀ = 1000011111010000₂ (bit dấu 1, phần trị 000011111010000₂ = 2000)

So sánh từng bit:
Vị trí 15: 1 = 1 (cùng âm)
Vị trí 14: 0 = 0
Vị trí 13: 0 = 0
Vị trí 12: 0 = 0
Vị trí 11: 0 = 0
Vị trí 10: 0 = 0
Vị trí 9: 0 = 0
Vị trí 8: 0 = 0
Vị trí 7: 0 = 0
Vị trí 6: 0 = 0
Vị trí 5: 0 = 0
Vị trí 4: 0 = 0
Vị trí 3: 0 < 1 → phần trị của -1000 (000001111101000₂) < phần trị của -2000 (000011111010000₂)
→ Trong Signed Magnitude, số âm có phần trị nhỏ hơn thì lớn hơn
→ -1000 > -2000

Kết quả: -1000 > -2000 (16 bit có dấu, Signed Magnitude)
```

---

## BÀI TẬP TỰ RÈN LUYỆN (20 bài)

### Cấp độ cơ bản (1-10):
1. So sánh 3 và 5 trong 4 bit có dấu
2. So sánh 7 và 9 trong 4 bit có dấu
3. So sánh 12 và 4 trong 4 bit có dấu
4. So sánh 15 và 1 trong 4 bit có dấu
5. So sánh 0 và 0 trong 4 bit có dấu
6. So sánh 50 và 100 trong 8 bit có dấu
7. So sánh 50 và 100 trong 8 bit có dấu (đã có ở Bài 6)
8. So sánh 100 và 50 trong 8 bit có dấu (đã có ở Bài 7)
9. So sánh 127 và 0 trong 8 bit có dấu
10. So sánh 0 và 127 trong 8 bit có dấu

### Cấp độ trung bình (11-15):
11. So sánh 1000 và 5000 trong 16 bit có dấu
12. So sánh 10000 và 20000 trong 16 bit có dấu
13. So sánh 20000 và 10000 trong 16 bit có dấu
14. So sánh 30000 và 5000 trong 16 bit có dấu
15. So sánh 32767 và 0 trong 16 bit có dấu

### Cấp độ nâng cao (16-20):
16. So sánh 1000000 và 5000000 trong 32 bit có dấu
17. So sánh 10000000 và 20000000 trong 32 bit có dấu
18. So sánh 50000000 và 10000000 trong 32 bit có dấu
19. So sánh 100000000 và 50000000 trong 32 bit có dấu
20. So sánh 2147483647 và 0 trong 32 bit có dấu

---

## MẸO VÀ LƯU Ý

### MẸO LÀM NHANH:
- **So sánh từng bit** từ trái sang phải
- **Bit đầu tiên khác nhau** quyết định kết quả
- **Tất cả bit giống nhau** → bằng nhau
- **Nhớ** bit trái có trọng số cao nhất

### LƯU Ý QUAN TRỌNG:
- **Bit trái** có trọng số cao nhất
- **Bit phải** có trọng số thấp nhất
- **So sánh** từ trái sang phải
- **Kiểm tra** phạm vi giá trị

### CÁCH KIỂM TRA:
- **So sánh** từng bit một
- **Xác định** bit đầu tiên khác nhau
- **Kết luận** dựa trên bit đó
- **Kiểm tra** bằng máy tính

### CÁC TRƯỜNG HỢP ĐẶC BIỆT:
- **A = A** (bằng chính nó)
- **A > 0** (bất kỳ số nào > 0)
- **0 = 0** (0 bằng 0)

### PHƯƠNG PHÁP NHANH:
1. **So sánh** từng bit
2. **Xác định** bit đầu tiên khác nhau
3. **Kết luận** dựa trên bit đó
4. **Ghi** kết quả cuối cùng

### VÍ DỤ ỨNG DỤNG:
```cpp
// So sánh số nguyên có dấu
enum ComparisonResult {
    LESS_THAN,
    EQUAL,
    GREATER_THAN
};

ComparisonResult compareSigned(int a, int b, int bits) {
    int maxValue = (1 << (bits - 1)) - 1;
    int minValue = -(1 << (bits - 1));
    
    // Kiểm tra phạm vi
    if (a > maxValue || a < minValue || b > maxValue || b < minValue) {
        return EQUAL; // Invalid comparison
    }
    
    if (a < b) return LESS_THAN;
    if (a > b) return GREATER_THAN;
    return EQUAL;
}

// So sánh bit
bool isEqual(int a, int b, int bits) {
    int mask = (1 << bits) - 1;
    return (a & mask) == (b & mask);
}

bool isGreater(int a, int b, int bits) {
    int mask = (1 << bits) - 1;
    return (a & mask) > (b & mask);
}

bool isLess(int a, int b, int bits) {
    int mask = (1 << bits) - 1;
    return (a & mask) < (b & mask);
}
```

### ỨNG DỤNG THỰC TẾ:
- **Máy tính**: Signed comparison
- **Assembly**: CMP instruction
- **Counters**: Loop counters, array indices
- **Memory addressing**: Address calculations
- **Hash functions**: Checksum calculations

---

**Tài liệu tham khảo:**
- Chương 2: Biểu diễn dữ liệu trên máy tính
- Phần 4.1: Biểu diễn số nguyên có dấu
