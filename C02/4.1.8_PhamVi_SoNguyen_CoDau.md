...
**Lưu ý quan trọng:** File này mô tả phạm vi giá trị của số có dấu sử dụng phương pháp **SIGNED MAGNITUDE** (bit dấu + phần trị). Để xác định phạm vi giá trị của số có dấu bằng phương pháp **TWO'S COMPLEMENT** (bù 2 - chuẩn máy tính), xem các file trong phần 4.2.
...
1. **Tràn số**: giá trị > 2^(n-1) - 1 hoặc < -(2^(n-1) - 1)
...
Kết quả: Phạm vi của số nguyên 4 bit có dấu (Signed Magnitude) là -7 đến 7
Lưu ý: Số 1000₂ là -0, không phải -8
```
...
Kết quả: Phạm vi của số nguyên 8 bit có dấu (Signed Magnitude) là -127 đến 127
Lưu ý: Số 10000000₂ là -0, không phải -128
```
...
Kết quả: Phạm vi của số nguyên 16 bit có dấu (Signed Magnitude) là -32767 đến 32767
```
...
Bước 4: Xác định phạm vi
-2147483647 ≤ giá trị ≤ 2147483647

Kết quả: Phạm vi của số nguyên 32 bit có dấu (Signed Magnitude) là -2147483647 đến 2147483647
```
...
**Lời giải:**
```
Bước 1: Xác định phạm vi 4 bit (Signed Magnitude)
-7 ≤ giá trị ≤ 7
...
**Lời giải:**
```
Bước 1: Xác định phạm vi 8 bit (Signed Magnitude)
-127 ≤ giá trị ≤ 127
...
### Bài 10: Kiểm tra tràn số: -7 trong 4 bit có dấu
...
Kết quả: -7 trong 4 bit có dấu không tràn số, kết quả thực tế là -7
Lưu ý: -8 không tồn tại trong Signed Magnitude 4 bit (1000₂ là -0)
```
...
### MẸO LÀM NHANH:
- **Nhớ công thức** phạm vi: -(2^(n-1) - 1) đến 2^(n-1) - 1
- **Tính giá trị tối đa** = 2^(n-1) - 1
- **Tính giá trị tối thiểu** = -(2^(n-1) - 1)
- **Kiểm tra tràn** = giá trị > 2^(n-1) - 1 hoặc < -(2^(n-1) - 1)
...
- **-(2^(n-1) - 1)**: giá trị tối thiểu (Signed Magnitude)
...
int getMinValue(int bits) {
    int maxValue = (1 << (bits - 1)) - 1;
    return -maxValue;  // Signed Magnitude: -(2^(n-1) - 1)
}
```
